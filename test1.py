...
from RAW.new import model_flow, model_IO, cond_part, mfs
sb = mfs().from_data(X = sample1, Y = y)
sb.binning(mode = "B", ruleB = 7, pass_error=True, remain_ticks=False)
conds = cond_part(pd.to_datetime(sb.X["dt"]), 0.5)
sb.draw_binning(conds = conds)

sb.model_IO.save_binning_html()
sb.model_IO.save_binning_png()
sb.save_binning()




getattr(sb.model_IO.model, "binning_tools")
sb.save_binning()
sb.save_index()
sb.save_cluster()
sb.save_cluster()
...
sb.model_IO.version_node[2]

sb.a0 = 2
sb.a1 = 3
sb.a2 = 7
sb.model_IO.lv_version

sb.model_IO.lv_saved

from types import MethodType
sb.model_IO.save_data = MethodType(model_IO.save_data, sb.model_IO)

sb.model_IO.lv_saved
sb.model_IO.lv
pd.Series(sb.model_IO.index_saved)
...
sb.binning_tools
sb.model_IO.formatted("binning_tools")
sb.model_IO.load("binning_tools")

pickle.dumps({"a": 1}).hex()
pickle.dumps("asdf")
